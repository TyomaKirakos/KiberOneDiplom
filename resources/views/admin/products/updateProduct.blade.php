...
@section('content')
    <main class="main container">
        <section class="auth-block block">
            <h1 class="auth-block__heading heading">Редактирование товара</h1>
            <form action="{{ route('ProductUpdate', $product) }}" method="POST" class="auth-block__auth-form">
                @csrf
                <div class="auth-form__auth-inputs">
                    <label for="name" class="auth-inputs__label just-text">Название</label>
                    <input type="text" class="auth-inputs__input just-text @error('name') invalid-input @enderror" name="name" id="name" value="{{ $product->name }}">
                    @error('name')
                ...
                <div class="auth-form__auth-inputs">
                    ...
                    <input type="number" class="auth-inputs__input just-text @error('price') invalid-input @enderror" name="price" id="price" value="{{ $product->price }}">
                    @error('price')
